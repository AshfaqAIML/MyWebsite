...
export async function GET(req: NextRequest) {
  ...
  const [totalBooks, totalHighlights, totalNotes, totalBookmarks, totalFlashcards, sessions, completedBooks] = await Promise.all([
    ...
    prisma.studySession.findMany({ where: { userId }, orderBy: { date: "desc" }, take: 100 }),
    prisma.readingProgress.count({ where: { userId, completed: true } }),
  ]);

  const totalStudyMinutes = sessions.reduce((sum: number, s: any) => sum + s.duration, 0);

  const now = new Date();
  let streak = 0;
  const dates = new Set(sessions.map((s: any) => s.date.toISOString().slice(0, 10)));
  for (let i = 0; i < 365; i++) {
