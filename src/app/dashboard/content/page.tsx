  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">Track your published content and its performance.</p>
      </div>

      <div className="grid grid-cols-4 gap-4 mb-8">
        <div className="p-5 rounded-2xl border border-zinc-200/50 dark:border-zinc-700/50 bg-white dark:bg-zinc-900">
          <div className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400 mb-2"><Eye className="h-4 w-4" /> Total Views</div>
          <p className="text-3xl font-bold text-zinc-900 dark:text-zinc-50">{totalViews}</p>
        </div>
        <div className="p-5 rounded-2xl border border-zinc-200/50 dark:border-zinc-700/50 bg-white dark:bg-zinc-900">
          <div className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400 mb-2"><Download className="h-4 w-4" /> Total Downloads</div>
          <p className="text-3xl font-bold text-zinc-900 dark:text-zinc-50">{totalDownloads}</p>
        </div>
        <div className="p-5 rounded-2xl border border-zinc-200/50 dark:border-zinc-700/50 bg-white dark:bg-zinc-900">
          <div className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400 mb-2"><FileText className="h-4 w-4" /> Publications</div>
          <p className="text-3xl font-bold text-zinc-900 dark:text-zinc-50">{publications.length}</p>
        </div>
        <div className="p-5 rounded-2xl border border-zinc-200/50 dark:border-zinc-700/50 bg-white dark:bg-zinc-900">
          <div className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400 mb-2"><BookOpen className="h-4 w-4" /> Books & Articles</div>
      ...
      <div className="space-y-3">
        {content.map((item) => (
          <div key={item.id} className="p-4 rounded-xl border border-zinc-200/50 dark:border-zinc-700/50 bg-white dark:bg-zinc-900">
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-400">{item.platform} · {item.views} views · {item.downloads} downloads</p>
          </div>
        ))}
        {content.length === 0 && (
          <p className="text-center py-16 text-zinc-500 dark:text-zinc-400">No content tracked yet.</p>
        )}
      </div>
    </div>
  );
